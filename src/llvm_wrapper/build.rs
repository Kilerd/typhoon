use llvm_sys::prelude::{LLVMValueRef, LLVMBuilderRef, LLVMTypeRef, LLVMContextRef, LLVMModuleRef, LLVMBasicBlockRef};
use llvm_sys::core::{LLVMBuildStore, LLVMBuildLoad, LLVMBuildRet, LLVMBuildAlloca, LLVMBuildGEP, LLVMModuleCreateWithName, LLVMAddFunction, LLVMAppendBasicBlockInContext, LLVMPositionBuilderAtEnd, LLVMBuildICmp, LLVMBuildCondBr, LLVMBuildBr, LLVMBuildPhi, LLVMAddIncoming};
use std::ffi::CString;
...
impl Build {
    ...
    pub fn append_block(context: LLVMContextRef, func: LLVMValueRef, name: &str) -> LLVMBasicBlockRef {
        let name = CString::new(name).unwrap();
        unsafe {
            LLVMAppendBasicBlockInContext(context, func, name.as_ptr())
        }
    }
    pub fn add_func_to_module(module: LLVMModuleRef, func_name: &str, func_ty: LLVMTypeRef) -> LLVMValueRef {
        let name = CString::new(func_name).unwrap();
        unsafe {
            LLVMAddFunction(module, name.as_ptr(), func_ty)
        }
    }
    ...
    pub fn cond_br(builder: LLVMBuilderRef, if_: LLVMValueRef, then_block: LLVMBasicBlockRef, else_block: LLVMBasicBlockRef) -> LLVMValueRef {
        unsafe {
            LLVMBuildCondBr(builder, if_, then_block, else_block)
        }
    }
    pub fn cmp(op: LLVMIntPredicate, lhs: LLVMValueRef, rhs: LLVMValueRef, name: &str, builder: LLVMBuilderRef) -> LLVMValueRef {
        let name = CString::new(name).unwrap();
        unsafe {
            LLVMBuildICmp(builder, op, lhs, rhs, name.as_ptr())
        }
    }

    pub fn phi(builder: LLVMBuilderRef, ty: LLVMTypeRef, incoming: Vec<(LLVMValueRef, LLVMBasicBlockRef)>) -> LLVMValueRef {
        let (mut values, mut blocks): (Vec<LLVMValueRef>, Vec<LLVMBasicBlockRef>) = incoming.into_iter().unzip();
        unsafe {
    ...
    pub fn declare_struct(name: &str, typ: LLVMTypeRef, fields: &mut Vec<(u32, LLVMValueRef)>, builder: LLVMBuilderRef, context: LLVMContextRef) -> LLVMValueRef {
    ...
    pub fn gep(ty: LLVMValueRef, idx: u32, builder: LLVMBuilderRef, context: LLVMContextRef) -> LLVMValueRef {
        unsafe {
            let mut idx_vec = vec![Literal::uint32(0, context), Literal::uint32(idx, context)];
            LLVMBuildGEP(builder, ty, idx_vec.as_mut_ptr(), idx_vec.len() as u32, c_str!("gep_"))
        }
    }
